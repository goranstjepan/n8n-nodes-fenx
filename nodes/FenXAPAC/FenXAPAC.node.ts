...
export class FenXAPAC implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Fen-X APAC',
		name: 'fenXAPAC',
		icon: 'file:FenX.png',
		group: ['transform'],
		version: 1,
...
export class FenXToken {
	...
	static async getToken(func: IExecuteFunctions){
		...
		if(FenXToken.tokenExpires <= currentDate){
			let auth = await func.getCredentials('fenXAPAC');
	...
	private static async generateNewToken(func: IExecuteFunctions, clientId:string, clientSecret:string){
		...
		let url = "https://identity.apac1.fenergox.com/connect/token";
