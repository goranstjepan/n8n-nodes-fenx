...
requestOptions.uri = 'https://api.fenergox.com/bulkloadcommand/api/jobs';

requestOptions.body = base.getNodeParameter('request', 0) as string; requestOptions.json = true;break;
case 'CancelJob': id = base.getNodeParameter('id', 0) as string;
requestOptions.method = 'POST';
requestOptions.uri = 'https://api.fenergox.com/bulkloadcommand/api/jobs/{id}/cancellation'.replace('{id}', id);

break;
case 'UpdateJob': id = base.getNodeParameter('id', 0) as string;
requestOptions.method = 'POST';
requestOptions.uri = 'https://api.fenergox.com/bulkloadcommand/api/jobs/{id}'.replace('{id}', id);

requestOptions.body = base.getNodeParameter('request', 0) as string; requestOptions.json = true;break;
case 'SetJobUploadedBy': id = base.getNodeParameter('id', 0) as string;
requestOptions.method = 'POST';
requestOptions.uri = 'https://api.fenergox.com/bulkloadcommand/api/jobs/{id}/uploadedBy'.replace('{id}', id);
...
requestOptions.uri = 'https://api.fenergox.com/bulkloadcommand/api/jobs/{jobId}/data-source-upload-signed-url/{type}'.replace('{jobId}', jobId).replace('{type}', type);
